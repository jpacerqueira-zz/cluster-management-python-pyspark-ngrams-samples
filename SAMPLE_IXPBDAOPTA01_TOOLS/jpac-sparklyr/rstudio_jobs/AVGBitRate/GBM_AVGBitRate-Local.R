...
#
# h20.init with JVM of >=2GB and <= 16GB mem. 
h2o.init(ip = "localhost", port = 54321, startH2O = TRUE,
         forceDL = FALSE, enable_assertions = TRUE, license = NULL,
#         nthreads = -1, max_mem_size = NULL, min_mem_size = "1g",
         nthreads = 2, max_mem_size = "16g" , min_mem_size = "2g",
...
conviva_file <- read.csv("/home/analyticsdb/projects/r-studio/AVG_Bitrate/conviva11-1asset.csv")
head(conviva_file)
...
conviva_csv_df <- spark_read_csv(sc,"conviva11","hdfs://bda-ns//user/analyticsdb/conviva11-1asset.csv")
head(conviva_csv_df)
...
#TRIAL 1-DeepLearningBooklet#                      distribution="quantile", quantile_alpha=0.8,
#TRIAL 1-DeepLearningBooklet#                      variable_importances = TRUE,
...
tmpdir_model <- "h2omodels-v5-avg-bitrate-dl"
dir.create(tmpdir_model)
# print the centroid statistics
h2o.centroid_stats(dl_fit)
# Save generic Model
h2o.saveModel(dl_fit, path = tmpdir_model)
# Export Model as a POJO with H2O
h2o.download_pojo(dl_fit, path = tmpdir_model)
# Export Model as a MOJO with H2O
h2o.download_mojo(dl_fit, path = tmpdir_model)
...
#
# New Split
splits <- h2o.splitFrame(conviva_hf, seed = 99999)
...
# GBM hyperparamters
gbm_params1 <- list(learn_rate = c(0.1,0.5, 1.0),
                   max_depth = c(3, 5, 9),
                    sample_rate = c(0.8,0.9, 1.0),
                     nbins = c(8, 12, 16, 24, 32),
...
# Building Energy Efficiency: Tuned GBM | page 153
models <- lapply(gbm_grid1@model_ids, h2o.getModel)
#models
mse_sd <- t(sapply(models, function(m){m@model$cross_validation_metrics_summary["mse",c("mean","sd")]}))
cbind( as.data.frame(gbm_grid1@summary_table), mse_sd)
...
# GBM hyperparamters
# Second Parameter Grid with more parameters
# Expensive Params for Grid , takes 10h to obtain models!!!
#
gbm_params2 <- list(learn_rate = c(0.5, 1.0),
                    max_depth = c(3, 5, 9),
#                   min_rows = c(1,2,5,10),
                    sample_rate = c(0.9,0.95),
                    nbins = c(8,12,16,24,32),
                    col_sample_rate = c(0.1,0.5, 0.9, 1.0))
#
#
#
#