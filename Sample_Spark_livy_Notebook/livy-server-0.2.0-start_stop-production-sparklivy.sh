...
export HUE_USER=sparklivy
export HUE_SECRET_KEY=‘::q
’
export USER_HOME=/home/sparklivy
export PRINCIPAL=sparklivy/ixpbda@BDA.PERFORMGROUP.COM
export KEYTAB=${HOME}/.keytabs/sparklivy.keytab


# for ${HUE_USER} Kerberos credentials
export KRB5CCNAME=/tmp/krb5cc_$(id -u ${HUE_USER})
kinit -kt ${USER_HOME}/.keytabs/${HUE_USER}.keytab ${HUE_USER}/ixpbda@BDA.PERFORMGROUP.COM -c ${KRB5CCNAME}
klist ${KRB5CCNAME}

#rm -rf /usr/lib/livy-server-0.2.0/logs
#mkdir /usr/lib/livy-server-0.2.0/logs
touch /usr/lib/livy-server-0.2.0/logs/livy-sparklivy-server.out
